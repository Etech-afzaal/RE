/**
 * Add price_currency (PKR | USD) on properties. Amount stays in `price`.
 * Run with `npm run migrate:property-price-currency` (add `-- --down` to roll back).
 */
const fs = require("fs");
const path = require("path");
const mysql = require("mysql2/promise");

const MIGRATION_ID = "012_property_price_currency";
const COLUMN = "price_currency";
const COLUMN_SQL =
  "ENUM('PKR','USD') NOT NULL DEFAULT 'PKR' AFTER price";
...
async function migrateUp(conn) {
  if (await columnExists(conn, COLUMN)) {
    ...
    await conn.query(
      `ALTER TABLE properties ADD COLUMN ${COLUMN} ${COLUMN_SQL}`,
    );
  ...
  console.log("Property price currency migration complete.");
}
...
async function migrateDown(conn) {
  ...
  console.log("Property price currency migration rolled back.");
}

/**
 * Add properties.is_featured for agent-controlled homepage hero listings.
 * Run with `npm run migrate:property-is-featured` (add `-- --down` to roll back).
 */
const fs = require("fs");
const path = require("path");
const mysql = require("mysql2/promise");

const MIGRATION_ID = "019_property_is_featured";
const COLUMN = "is_featured";
const INDEX = "idx_properties_agent_featured";
...
async function migrateUp(conn) {
  if (await columnExists(conn, COLUMN)) {
    ...
    await conn.query(
      `ALTER TABLE properties
       ADD COLUMN ${COLUMN} BOOLEAN NOT NULL DEFAULT FALSE AFTER status`,
    );
    console.log(`+ properties.${COLUMN}`);
  }

  if (await indexExists(conn, INDEX)) {
    console.log(`= ${INDEX} already exists`);
  } else {
    await conn.query(
      `CREATE INDEX ${INDEX} ON properties(agent_id, is_featured, status)`,
    );
    console.log(`+ ${INDEX}`);
  }
  ...
  console.log("Property is_featured migration complete.");
}

async function migrateDown(conn) {
  if (await indexExists(conn, INDEX)) {
    await conn.query(`DROP INDEX ${INDEX} ON properties`);
    console.log(`- ${INDEX}`);
  }
  if (await columnExists(conn, COLUMN)) {
  ...
  console.log("Property is_featured migration rolled back.");
}

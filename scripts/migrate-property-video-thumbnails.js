/**
 * Add thumbnail_url on property_videos for lazy-loaded posters.
 * Run with `npm run migrate:property-video-thumbnails` (add `-- --down` to roll back).
 */
const fs = require("fs");
const path = require("path");
const mysql = require("mysql2/promise");

const MIGRATION_ID = "014_property_video_thumbnails";
const TABLE = "property_videos";
const COLUMN = "thumbnail_url";
const COLUMN_SQL = "VARCHAR(500) NULL AFTER video_url";
...
async function migrateUp(conn) {
  ...
  console.log("Property video thumbnails migration complete.");
}
...
async function migrateDown(conn) {
  ...
  console.log("Property video thumbnails migration rolled back.");
}

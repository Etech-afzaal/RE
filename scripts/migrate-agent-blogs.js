/**
 * Agent blogs table.
 * Run with `npm run migrate:agent-blogs` (add `-- --down` to roll back).
 */
const fs = require("fs");
const path = require("path");
const mysql = require("mysql2/promise");

const MIGRATION_ID = "031_agent_blogs";
...
async function migrateUp(conn) {
  if (!(await tableExists(conn, "blogs"))) {
    ...
    console.log("  blogs already exists");
  }
  ...
  console.log("Agent blogs migration complete.");
}

async function migrateDown(conn) {
  if (await tableExists(conn, "blogs")) {
    await conn.query("DROP TABLE blogs");
    console.log("- blogs");
  }

  await conn.query("DELETE FROM schema_migrations WHERE id = ?", [MIGRATION_ID]);
  console.log("Agent blogs migration rolled back.");
}

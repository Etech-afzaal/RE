...
test("old clients omitting insights preserve the original marketing columns", () => {
  const current = { why_this_home: '["Original insight"]', investment_insights: ["Rental income"] };
  const result = data.preparePropertyDataSave({}, "sale", "house", null, current);
  ...
  const added = data.preparePropertyDataSave({ property_data: { landInfo: { front: 30 } } }, "sale", "house", null, current);

...
test("edit legacy shop preserves subtype, populates Commercial Type, validates edits and retains old insights", async () => {
  ...
  await ui.click("Save Draft");
  assert.ok(!ui.requests.some(r => r.method === "PUT"));
...
test("reject malformed JSON, invalid numbers/options, HTML and irrelevant file amenities", () => {
  ...
  assert.equal(data.normalizePropertyData({ plotInfo: { amenities: ["Water"] } }, "sale", "file").ok, false);

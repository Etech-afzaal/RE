...
test("media categories vary only for plots/files and retain selections across type switches", () => {
  ...
  const Select = load("@/components/ImageCategorySelect").default;
  ...
  for (const [kind, media] of Object.entries(expected)) {
    for (const [mediaType, labels] of Object.entries(media)) {
      ...
      assert.deepEqual(tree.filter(n => n.type === "option").map(n => n.props.children), ["Select category", ...labels]);
      const select = tree.find(n => n.type === "select");
  ...
  for (const value of ["living_room", "File / Document", "My custom view", null]) {
    for (const kind of ["house", "plots", "file", "house"]) {
      ...
      const selected = tree.find(n => n.type === "select").props.value;
      assert.ok(tree.some(n => n.type === "option" && n.props.value === selected));
      if (selected === "other") assert.equal(tree.find(n => n.type === "input").props.value, value);

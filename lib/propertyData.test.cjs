...
const load = loader();
test("wizard allows blank plot size and price but validates entered values", () => {
  const { getPropertyWizardFieldError } = load("@/lib/validators/propertyValidator");
  for (const field of ["size_value", "price"]) {
    for (const value of ["", null, undefined, "10"]) {
      assert.equal(getPropertyWizardFieldError(field, { [field]: value }), null);
    }
    for (const value of ["0", "-1", "abc"]) {
      assert.ok(getPropertyWizardFieldError(field, { [field]: value }));
    }
  }
});
test("all property measurements and counts reject zero but accept positive values", () => {
  const { normalizePropertyData } = load("@/lib/propertyData");
  for (const [section, keys] of Object.entries({ landInfo: ["front", "depth", "roadWidth"], propertyDetails: ["coveredArea", "floors", "lounge", "kitchens", "bedrooms", "bathrooms"] })) {
    for (const key of keys) {
      for (const value of [0, "0", "00", "0.00"]) {
        assert.equal(normalizePropertyData({ [section]: { [key]: value } }, "sale", "house").ok, false, `${key}: ${value}`);
      }
      for (const value of ["", 1, 10]) {
        assert.equal(normalizePropertyData({ [section]: { [key]: value } }, "sale", "house").ok, true, `${key}: ${value}`);
      }
      if (["front", "depth", "roadWidth", "coveredArea"].includes(key)) {
        assert.equal(normalizePropertyData({ [section]: { [key]: "0.1" } }, "sale", "house").ok, true);
      }
    }
  }
});
test("covered area rejects zero while remaining optional and accepting positive decimals", () => {
  const { normalizePropertyData } = load("@/lib/propertyData");
  for (const kind of ["house", "apartment", "commercial"]) {
    for (const coveredArea of [0, "0", "000", "0.00"]) {
      const result = normalizePropertyData({ propertyDetails: { coveredArea } }, "sale", kind);
      assert.equal(result.ok, false);
      assert.match(result.error, /greater than 0/);
    }
    for (const coveredArea of ["", null, "0.25", 2500]) {
      assert.equal(normalizePropertyData({ propertyDetails: { coveredArea } }, "sale", kind).ok, true);
    }
  }
});
test("numeric fields collapse leading zeros and preserve decimals and digit limits", () => {
  const NumberInput = load("@/components/agent-portal/PropertyNumberInput").default;
  for (const [raw, expected] of [["000000000", "0"], ["00012", "12"], ["0.25", "0.25"], ["120", "120"]]) {
    let value;
    const input = inputNodes(NumberInput({ value: "0", onChange: next => { value = next; } })).find(n => n.type === "input");
    const target = { value: raw };
    input.props.onChange({ target });
    assert.equal(value, expected);
    assert.equal(target.value, expected);
  }
  const { sanitizeWizardDigitInput } = load("@/lib/validators/propertyValidator");
  assert.equal(sanitizeWizardDigitInput("000000000", 6).value, "0");
  assert.equal(sanitizeWizardDigitInput("000123", 6).value, "123");
  assert.ok(sanitizeWizardDigitInput("1234567", 6).limitError);
});

test("draft saves reject zero price and size but allow empty and positive values", () => {
  const { validatePropertyDraftInput } = load("@/lib/validators/propertyValidator");
  for (const field of ["size_value", "price"]) {
    for (const value of [0, "0", "000000", "0.00"]) {
      const result = validatePropertyDraftInput({ title: "Test property", [field]: value });
      assert.equal(result.ok, false);
      assert.equal(result.field, field);
    }
    for (const value of ["", "0.25", "12"]) {
      assert.equal(validatePropertyDraftInput({ title: "Test property", [field]: value }).ok, true);
    }
  }
});

test("media categories vary only for plots/files and retain selections across type switches", () => {
  const { propertyMediaCategoryGroups, IMAGE_CATEGORY_GROUPS } = load("@/lib/imageCategories");
  const Select = loader({ react: { useId: () => "category-options" } })("@/components/ImageCategorySelect").default;
  const nodes = node => !node || typeof node !== "object" ? [] :
    Array.isArray(node) ? node.flatMap(nodes) : [node, ...nodes(node.props?.children)];
  for (const kind of ["house", "apartment", "commercial", "future-type", undefined]) {
    for (const media of ["image", "video"]) {
      assert.equal(propertyMediaCategoryGroups(kind, media), IMAGE_CATEGORY_GROUPS);
    }
  }
  const expected = {
    plots: {
      image: ["Front / Main View", "Road View", "Location / Surroundings", "Plot Boundary", "Development / Site", "Map / Location Plan", "Other"],
      video: ["Plot Walkthrough", "Location / Surroundings", "Road / Access", "Development / Site", "Other"],
    },
    file: {
      image: ["File / Document", "Location / Map", "Project / Development", "Payment / Installment Plan", "Other"],
      video: ["Project Overview", "Location / Surroundings", "Development / Site", "Other"],
    },
  };
  for (const [kind, media] of Object.entries(expected)) {
    for (const [mediaType, labels] of Object.entries(media)) {
      let saved;
      const tree = nodes(Select({ propertyKind: kind, mediaType, value: labels[0], onChange: value => { saved = value; } }));
      assert.deepEqual(tree.filter(n => n.type === "option").map(n => n.props.children), ["Select category", ...labels.filter(label => label !== "Other")]);
      assert.ok(tree.some(n => n.type === "optgroup"));
      tree.find(n => n.type === "select").props.onChange({ target: { value: labels[1] } });
      assert.equal(saved, labels[1]);
      const select = tree.find(n => n.type === "input");
      assert.equal(select.props.value, labels[0]);
      select.props.onChange({ target: { value: labels[1] } });
      assert.equal(saved, labels[1]);
    }
  }
  for (const value of ["living_room", "File / Document", "My custom view", null]) {
    for (const kind of ["house", "plots", "file", "house"]) {
      let calls = 0;
      const tree = nodes(Select({ propertyKind: kind, value, onChange: () => calls++ }));
      const selected = tree.find(n => n.type === "input").props.value;
      assert.equal(selected, value === "living_room" ? "Living Room" : value ?? "");
      assert.equal(calls, 0, "Rendering/type switching must not rewrite saved categories");
    }
  }
});

test("editable media categories support custom text, presets, clearing and disabled fields", () => {
  const Select = loader({ react: { useId: () => "category-options" } })("@/components/ImageCategorySelect").default;
  for (const mediaType of ["image", "video"]) {
    let saved;
    const input = inputNodes(Select({ mediaType, value: "living_room", disabled: true, onChange: value => { saved = value; } })).find(n => n.type === "input");
    assert.equal(input.props.value, "Living Room");
    assert.equal(input.props.disabled, true);
    assert.equal(input.props.maxLength, 60);
    for (const [text, expected] of [["Swimming Pool", "Swimming Pool"], ["Master Bedroom", "master_bedroom"], ["Living Room ", "Living Room "], ["Living Room View", "Living Room View"], ["", null], ["  ", null]]) {
      input.props.onChange({ target: { value: text } });
      assert.equal(saved, expected);
      const updated = inputNodes(Select({ mediaType, value: saved, onChange() {} })).find(n => n.type === "input");
      assert.equal(updated.props.value, text.trim() ? text : "");
    }
  }
});
...
test("numeric inputs block pasted letters and exponents while allowing valid numbers", () => {
  const NumberInput = load("@/components/agent-portal/PropertyNumberInput").default;
  for (const step of [1, 0.01]) {
    const input = inputNodes(NumberInput({ value: "2", step, onChange() {} })).find(n => n.type === "input");
    for (const text of ["e", "E", "1e3", "2E4", "abc", "12abc", "-1", "+2", "1.234", "25", "1.5"]) {
      let prevented = false;
      input.props.onPaste({ clipboardData: { getData: () => text }, preventDefault() { prevented = true; } });
      assert.equal(prevented, !(text === "25" || (step !== 1 && text === "1.5")), `step ${step}: paste ${text}`);
    }
  }
});
...
async function editHarness(row) {
  ...
  const migrated = { ...data.readPropertyData(row.property_data) };
  if (!Object.hasOwn(migrated, "listing_type")) migrated.listing_type = row.property_type;
  if (!Object.hasOwn(migrated, "property_type")) migrated.property_type = row.property_subtype;
  migrated.location ||= Object.fromEntries(["city", "area", "phase", "address"].map(key => [key, row[key] ?? null]));
  migrated.insights ||= Object.fromEntries(data.INSIGHT_FIELDS.map(key => [key, row[key] ?? null]));
  api.records.set(1, { ...row, property_data: JSON.stringify(migrated), id: 1, agent_id: 7 });
  ...
  global.fetch = async (url, options = {}) => {
    ...
    return Response.json({ property: load("@/lib/propertyRecord").propertyRecord(api.records.get(1)) });
  };
  ...
  const input = label => {
    if (label === "Commercial Type") return { props: {
      value: nodes().find(n => n.props?.name === "commercial-type" && n.props.checked)?.props.value || "",
      onChange: event => find(n => n.props?.name === "commercial-type" && n.props.value === event.target.value).props.onChange(),
    } };
...
test("edit NULL/incomplete JSON preserves existing columns and unrelated JSON; switching clears type-specific fields", async () => {
  for (const raw of [null, {}, { propertyDetails: null }, { landInfo: { front: 35, surveyReference: "keep" }, insights: { externalNote: "keep" }, externalSection: { reference: 123 } }]) {
    ...
    if (raw == null) assert.equal(JSON.parse(stored).listing_type, "sale", "Migrated legacy identity is preserved");
...
test("edit legacy shop preserves subtype, populates Commercial Type, validates edits and retains old insights", async () => {
  ...
  assert.equal(JSON.parse(row.property_data).property_type, "shop");
...
for (const [listing, kind] of combos) test(`${listing}/${kind}: classification, dynamic tabs, validation, JSON sections and submission`, () => {
  ...
  const row = { ...form, property_data: { ...normalized.data, listing_type: form.propertyType, property_type: form.propertySubtype, location: { city: form.city, area: form.area, address: form.address } } };
  assert.equal(submission.validatePropertyForSubmission(row, 1).valid, true);
  assert.equal(submission.validatePropertyForSubmission(row, 0).valid, ["plots", "file"].includes(kind));
  assert.equal(submission.validatePropertyForSubmission({ ...row, property_data: { ...row.property_data, location: { ...row.property_data.location, area: "" } } }, 1).valid, false);
  assert.equal(submission.validatePropertyForSubmission({ ...row, price: 0 }, 1).valid, false);
  assert.equal(submission.validatePropertyForSubmission({ ...row, size_value: "", price: null }, 1).valid, true);
});
...
test("JSON round-trip, legacy edits, insight synchronization and nullable old properties", () => {
  ...
  assert.deepEqual(cleared.marketing.why_this_home, []);
...
test("partial JSON updates retain omitted fields/sections and explicit empty values clear fields", () => {
  const current = data.normalizePropertyData(examples, "sale", "commercial").data;
  const result = data.preparePropertyDataSave({ property_data: { landInfo: { front: "35", depth: "" } } }, "sale", "commercial", current);
  assert.equal(result.ok, true);
  assert.equal(result.data.landInfo.front, 35);
  assert.ok(!Object.hasOwn(result.data.landInfo, "depth"));
  assert.equal(result.data.landInfo.plotPosition, "Corner");
  assert.deepEqual(result.data.propertyDetails, current.propertyDetails);
  assert.deepEqual(result.data.commercialInfo, current.commercialInfo);
  assert.deepEqual(result.data.insights, current.insights);
  for (const raw of ["bad json", [], { landInfo: [] }, { landInfo: "{}" }]) {
    assert.equal(data.preparePropertyDataSave({ property_data: raw }, "sale", "house", current).ok, false);
  }
});

test("old clients omitting insights preserve the migrated JSON", () => {
  const current = { why_this_home: '["Original insight"]', investment_insights: ["Rental income"] };
  const result = data.preparePropertyDataSave({}, "sale", "house", { insights: current });
  assert.deepEqual(result.marketing.why_this_home, ["Original insight"]);
  assert.deepEqual(result.marketing.investment_insights, ["Rental income"]);
  const added = data.preparePropertyDataSave({ property_data: { landInfo: { front: 30 } } }, "sale", "house", { insights: current });
  assert.deepEqual(added.data.insights.why_this_home, ["Original insight"]);
});
...
for (const [listing, kind] of combos) test(`${listing}/${kind}: actual POST/PUT/submit handlers with isolated database stub`, async () => {
  ...
  assert.deepEqual(stored.insights.why_this_home, examples.insights.why_this_home);
  assert.ok(!Object.hasOwn(row, "why_this_home"), "No separate insight column written");
...
test("create/edit APIs reject malformed or non-object JSON bodies without database writes", async () => {
  const api = apiHarness();
  for (const json of [async () => { throw new SyntaxError("Invalid JSON"); }, async () => null, async () => [], async () => "text"]) {
    assert.equal((await api.create({ json })).status, 400);
    assert.equal((await api.update({ json }, { params: { id: 1 } })).status, 400);
  }
  assert.equal(api.sqlCalls.length, 0);
});

test("Other commercial type input saves, edits and renders custom text", async () => {
  const Fields = load("@/components/agent-portal/PropertyDataFields").default;
  const custom = { commercialType: "Other", commercialTypeOther: "Medical Center" };
  const fields = inputNodes(Fields({ section: "commercialInfo", kind: "commercial", data: custom, onChange() {} }));
  assert.ok(fields.some(node => node.props?.customValue === "Medical Center"));
  const hidden = inputNodes(Fields({ section: "commercialInfo", kind: "commercial", data: { ...custom, commercialType: "Office" }, onChange() {} }));
  assert.ok(!hidden.some(node => node.type === "input" && node.props.value === "Medical Center"));
  const api = apiHarness();
  const body = { ...base, propertyType: "sale", propertySubtype: "commercial", property_data: { commercialInfo: custom } };
  const created = await api.create({ json: async () => body });
  assert.equal(created.status, 200);
  const { propertyId } = await created.json();
  const row = api.records.get(propertyId);
  assert.deepEqual(JSON.parse(row.property_data).commercialInfo, custom);
  const updated = await api.update({ json: async () => ({ ...body, property_data: { commercialInfo: { ...custom, commercialTypeOther: "Hotel" } } }) }, { params: { id: propertyId } });
  assert.equal(updated.status, 200);
  const projected = load("@/lib/propertyRecord").propertyRecord(row);
  const details = load("@/lib/publicPropertyData").publicPropertyDetails(projected);
  assert.ok(details.sections.some(section => section.rows.some(item => item.label === "Commercial Type" && item.value === "Hotel")));
  assert.equal(data.normalizePropertyData({ commercialInfo: { ...custom, commercialTypeOther: "<script>bad</script>" } }, "sale", "commercial").ok, false);
  assert.ok(!Object.hasOwn(data.normalizePropertyData({ commercialInfo: { ...custom, commercialType: "Office" } }, "sale", "commercial").data.commercialInfo, "commercialTypeOther"));
});

test("commercial dropdown adds and selects a custom option only after Add", () => {
  const hooks = []; let cursor = 0, value = "", customValue = "";
  const Input = loader({ react: {
    ...
    useRef: () => ({ current: null }),
  } })("@/components/agent-portal/CommercialTypeInput").default;
  const render = () => { cursor = 0; return inputNodes(Input({ value, customValue, options: ["Office", "Shop", "Other"], onChange(type, custom) { value = type; customValue = custom; } })); };
  render().find(node => node.props?.value === "Other").props.onChange();
  assert.ok(render().some(node => node.props?.["aria-label"] === "Custom commercial type"));
  render().find(node => node.type === "button").props.onClick();
  assert.equal(value, "");
  assert.ok(render().some(node => node.props?.role === "alert"));
  render().find(node => node.props?.["aria-label"] === "Custom commercial type").props.onChange({ target: { value: "Medical Center" } });
  assert.equal(value, "", "Typing alone does not commit the type");
  render().find(node => node.type === "button").props.onClick();
  assert.equal(value, "Other"); assert.equal(customValue, "Medical Center");
  assert.equal(render().find(node => node.type === "summary").props.children, "Medical Center");
  assert.ok(render().find(node => node.props?.value === "Medical Center").props.checked);
  assert.ok(!render().some(node => node.props?.["aria-label"] === "Custom commercial type"));
  render().find(node => node.props?.value === "Office").props.onChange();
  assert.equal(value, "Office");
  assert.ok(render().some(node => node.props?.value === "Medical Center"), "Added option stays available during editing");
});
...
for (const [listing, kind] of combos) for (const action of ["Save Draft", "Submit For Approval", ...(listing === "sale" && kind === "commercial" ? ["Retry Save Draft"] : [])]) test(`${listing}/${kind}: wizard navigation, media payloads, insights and ${action}`, async () => {
  ...
  if (kind === "plots") {
    ui.change("Plot Type", "Commercial");
    assert.equal(ui.input("Plot Type").props.value, "Commercial");
  }
  const initialFields = ui.nodes();
  for (const label of ["City", "Area / Neighbourhood", "Phase/Sector", "Street Address"]) {
    assert.ok(initialFields.indexOf(ui.input(label)) < initialFields.indexOf(ui.input("Description")), `${label} precedes Description in Basic Information`);
  }
  assert.ok(!ui.nodes().some(n => n.type === "button" && ui.text(n).trim() === "Location"));
  ...
  await ui.click("Continue");
  if (["plots", "file"].includes(kind)) {
    assert.ok(ui.nodes().some(n => n.type === "input" && n.props.accept?.startsWith("video/")), "Plots and Files can continue without images");
    await ui.click("Back");
  }
  assert.ok(ui.nodes().some(n => n.type === "input" && n.props.accept?.startsWith("image/")), "Image uploads remain available");
  const imageFiles = [new File(["test"], "photo.jpg", { type: "image/jpeg" })];
  if (action === "Submit For Approval") {
    const dropZone = ui.find(n => typeof n.props?.onDrop === "function");
    let prevented = false;
    const dataTransfer = { files: imageFiles };
    dropZone.props.onDragOver({ preventDefault() {}, dataTransfer });
    assert.equal(dataTransfer.dropEffect, "copy");
    dropZone.props.onDrop({ preventDefault() { prevented = true; }, dataTransfer });
    assert.equal(prevented, true);
  } else {
    ui.find(n => n.type === "input" && n.props.type === "file").props.onChange({ target: { files: imageFiles, value: "" } });
  }
  await ui.tick();
  await ui.click("Continue");

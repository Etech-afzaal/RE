 ...
 *           properties draft|pending_approval|approved|rejected|under_contract|sold|hidden
 *
...
export const PROPERTY_PUBLIC_STATUS = PROPERTY_STATUS.PUBLISHED;
/** Statuses that remain visible on an agent's public listings page. */
export const PROPERTY_PUBLIC_LISTING_STATUSES = [
  PROPERTY_STATUS.PUBLISHED,
  PROPERTY_STATUS.UNDER_CONTRACT,
  PROPERTY_STATUS.SOLD,
];

/** Max published listings an agent may pin to their public homepage hero. */
export const MAX_FEATURED_PROPERTIES_PER_AGENT = 10;
...
export function toDbPropertyStatus(clientStatus) {
  if (clientStatus === "active") return PROPERTY_STATUS.PUBLISHED;
  return clientStatus;
}

/** Map DB property status → client/UI value. */
export function toClientPropertyStatus(dbStatus) {
  if (dbStatus === PROPERTY_STATUS.PUBLISHED) return "active";
  return dbStatus;
}
 ...
 * only the submit endpoint may set it. approved is reachable again from a
 * sold, hidden, or under-contract listing, i.e. re-listing an already cleared property.
 */
const AGENT_TRANSITIONS = {
  [PROPERTY_STATUS.DRAFT]: [PROPERTY_STATUS.DRAFT],
  [PROPERTY_STATUS.REQUIRES_UPDATES]: [PROPERTY_STATUS.REQUIRES_UPDATES, PROPERTY_STATUS.DRAFT],

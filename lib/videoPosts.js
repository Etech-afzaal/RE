...
const VIDEO_POST_SELECT =
  "id, agent_id, title, slug, description, video_source, video_url, youtube_video_id, thumbnail_url, status, created_at, updated_at";
...
export async function createVideoPost(agentId, data) {
  const result = await query(
    `INSERT INTO video_posts
      (agent_id, title, slug, description, video_source, video_url, youtube_video_id, thumbnail_url, status)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      agentId,
      data.title,
...
export async function updateVideoPost(agentId, videoPostId, data) {
  ...
  await query(
    `UPDATE video_posts
    SET title = ?, slug = ?, description = ?, video_source = ?, youtube_video_id = ?, thumbnail_url = ?, status = ?
     WHERE id = ? AND agent_id = ?`,
    [
      data.title,
      data.slug,
      data.description || null,
      data.video_source || existing.video_source || "UPLOAD",
      data.youtube_video_id ?? existing.youtube_video_id ?? null,
      Object.hasOwn(data, "thumbnail_url") ? data.thumbnail_url : existing.thumbnail_url,

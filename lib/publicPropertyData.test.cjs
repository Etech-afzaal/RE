...
const load = loader();
test("card type tags use Commercial/Plot selections only, with safe legacy fallbacks", () => {
  const label = load("@/lib/publicPropertyData").publicPropertyCardTypeLabel;
  const commercial = { property_type: "sale", property_subtype: "commercial" };
  assert.equal(label({ ...commercial, property_data: { commercialInfo: { commercialType: "Plaza" } } }, "Commercial"), "Plaza");
  assert.equal(label({ ...commercial, property_data: { commercialInfo: { commercialType: "Other", commercialTypeOther: "Medical Center" } } }, "Commercial"), "Medical Center");
  assert.equal(label({ property_type: "plot", property_subtype: "residential_plot", property_data: JSON.stringify({ plotInfo: { plotType: "Residential" } }) }, "Residential Plot"), "Residential");
  for (const subtype of ["house", "apartment", "file"]) {
    assert.equal(label({ property_type: "sale", property_subtype: subtype, property_data: { commercialInfo: { commercialType: "Plaza" }, plotInfo: { plotType: "Commercial" } } }, subtype), subtype);
  }
  for (const property_data of [null, {}, { commercialInfo: { commercialType: "Other" } }]) {
    assert.equal(label({ ...commercial, property_data }, "Commercial"), "Commercial");
  }
});
...
for (const [listing, kind] of combos) for (const legacy of [false, true]) test(`public detail ${listing}/${kind}: ${legacy ? "NULL fallback" : "dynamic JSON"}`, async () => {
  ...
  const property = { ...base, property_type: selected.propertyType, property_subtype: selected.propertySubtype, property_data: legacy ? { listing_type: selected.propertyType, property_type: selected.propertySubtype, insights: { why_this_home: ["Legacy lifestyle"] } } : JSON.stringify(json) };
  ...
  if (kind === "apartment") assert.equal(summary.props.sizeLabel, legacy ? "5 Marla" : "1500 sqft");
  ...
  if (legacy) {
    ...
    if (["house", "apartment", "commercial"].includes(kind)) {
      ...
      assert.ok(labels.includes("Front")); assert.ok(!labels.includes("Facing"));
...
test("detail measurements include units without duplicating existing units", () => {
  for (const roadWidth of [40, "40 ft", "40 ft+"]) {
    const details = mapping.publicPropertyDetails({ ...base, property_type: "sale", property_subtype: "house", property_data: {
      landInfo: { front: 30, depth: "75", roadWidth }, propertyDetails: { coveredArea: 2500, floors: 2 },
    } });
    const values = Object.fromEntries(details.sections.flatMap(section => section.rows.map(row => [row.label, row.value])));
    assert.equal(values["Front"], "30 ft");
    assert.equal(values["Depth"], "75 ft");
    assert.equal(values["Covered Area"], "2500 sqft");
    assert.equal(values["Road Width"], roadWidth === "40 ft+" ? "40 ft+" : "40 ft");
    assert.equal(values["Floors / Stories"], "2");
  }
});

test("apartment covered area includes sqft and returns null when unavailable", () => {
  for (const raw of [{ propertyDetails: { coveredArea: 2500 } }, JSON.stringify({ propertyDetails: { coveredArea: "2500" } })]) {
    assert.equal(mapping.apartmentCoveredAreaLabel({ ...base, property_data: raw }), "2500 sqft");
  }
  for (const value of [undefined, null, "", 0, -1, "invalid"]) {
    assert.equal(mapping.apartmentCoveredAreaLabel({ ...base, property_data: { propertyDetails: { coveredArea: value } } }), null);
  }
});
...
test("empty/incomplete JSON and both Commercial Type keys are safe; insights preserve explicit empty lists", () => {
  ...
  assert.equal(mapping.publicPropertyInsight({ property_data: {}, investment_insights: ["Old"] }, "investment_insights"), undefined);
...
test("saved empty JSON amenities and insights do not show generated fallback sections", async () => {
  const normalized = formData.normalizePropertyData({ propertyDetails: { amenities: [] }, insights: { why_this_home: [], property_highlights: [] } }, "sale", "house");
  const property = { ...base, property_type: "sale", property_subtype: "house", property_data: normalized.data };
  assert.deepEqual(mapping.publicPropertyDetails(property).amenities, []);
  assert.deepEqual(mapping.publicPropertyInsight(property, "why_this_home"), []);
  const tree = await pageLoader(property)({ params: { estate_name: "test", propertyId: "1" } });
  const all = nodes(tree);
  assert.ok(!all.some(n => n.props?.id === "amenities-heading"));
  assert.ok(!all.some(n => n.props?.id === "highlights-heading"));
  assert.equal(mapping.publicPropertyDetails({ ...property, property_data: null }).amenities, null);
  assert.deepEqual(mapping.publicPropertyInsight({ ...property, why_this_home: ["Stale column"] }, "why_this_home"), []);
});

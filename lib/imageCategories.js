...
// Use the existing custom-label storage format so public galleries need no changes.
const PROPERTY_MEDIA_CATEGORIES = {
  plots: {
    image: ["Front / Main View", "Road View", "Location / Surroundings", "Plot Boundary", "Development / Site", "Map / Location Plan"],
    video: ["Plot Walkthrough", "Location / Surroundings", "Road / Access", "Development / Site"],
  },
  file: {
    image: ["File / Document", "Location / Map", "Project / Development", "Payment / Installment Plan"],
    video: ["Project Overview", "Location / Surroundings", "Development / Site"],
  },
};

export function propertyMediaCategoryGroups(kind, mediaType = "image") {
  const labels = PROPERTY_MEDIA_CATEGORIES[kind]?.[mediaType];
  if (!labels) return IMAGE_CATEGORY_GROUPS;
  return [{
    id: `${kind}-${mediaType}`,
    label: kind === "plots" ? "Plot" : "File",
    categories: [...labels.map(label => ({ value: label, label })), { value: "other", label: "Other" }],
  }];
}

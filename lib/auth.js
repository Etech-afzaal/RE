...
import { isDatabaseConnectivityError, query } from "@/lib/db";
...
const AUTH_DATABASE_UNAVAILABLE = "AUTH_DATABASE_UNAVAILABLE";

async function queryForAuth(sql, params = []) {
  try {
    ...
    async jwt({ token, user }) {
      ...
      if (isAgentRole(token.role) && token.sub) {
        try {
          ...
          // Keep the existing JWT claims if MySQL is briefly unreachable.
          // Throwing here becomes JWT_SESSION_ERROR and clears the session.
          if (!isDatabaseConnectivityError(err)) {
            throw err;
          }
        }

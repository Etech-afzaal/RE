 ...
 * propertyType / propertySubtype are persisted; title encoding remains for legacy filters.
 */

import { formatPropertyLocation } from "@/lib/propertyLocation";
import { readPropertyData } from "@/lib/propertyData";
import {
  PROPERTY_TYPES as PROPERTY_TYPE_LIST,
...
export const PROPERTY_TYPES = new Set(PROPERTY_TYPE_LIST);
...
/** Live character caps for text wizard fields (Add Property). */
export const WIZARD_TEXT_LIMITS = {
  title: 50,
  city: 20,
  area: 20,
  phase: 20,
...
export function validatePropertyDraftInput(input = {}, opts = {}) {
  ...
  const fields = {
    title: validateLength(input.title, {
      ...
      max: WIZARD_TEXT_LIMITS.title,
    }),
    description: validateLength(input.description, {
      required: false,
      max: WIZARD_TEXT_LIMITS.description,
      multiline: true,
    }),
    city: validateLength(input.city, {
      required: false,
      max: WIZARD_TEXT_LIMITS.city,
    }),
    area: validateLength(input.area, {
      required: false,
      max: WIZARD_TEXT_LIMITS.area,
    }),
    phase: validateLength(input.phase, {
      required: false,
      max: WIZARD_TEXT_LIMITS.phase,
    }),
    address: validateLength(input.address, {
      required: false,
      max: WIZARD_TEXT_LIMITS.address,
    }),
  ...
  const rawType =
    input.propertyType ?? input.property_type ?? null;
  const rawSubtype =
    input.propertySubtype ?? input.property_subtype ?? null;
  if (
    (rawType != null && String(rawType).trim() !== "") ||
    (rawSubtype != null && String(rawSubtype).trim() !== "")
  ) {
    const combo = validatePropertyTypeSubtype(rawType, rawSubtype, {
      required: false,
    });
    if (!combo.ok) {
      return { ok: false, error: combo.error, field: combo.field };
    }
    if (combo.property_type) {
      fields.propertyType = { ok: true, value: combo.property_type };
      fields.property_type = { ok: true, value: combo.property_type };
    }
    if (combo.property_subtype) {
      fields.propertySubtype = { ok: true, value: combo.property_subtype };
      fields.property_subtype = { ok: true, value: combo.property_subtype };
    }
  }
...
const WIZARD_FIELD_STEP = {
  ...
  propertySubtype: PROPERTY_WIZARD_STEPS.BASIC,
 ...
 * @returns {{ value: string, limitError: string|null }}
 */
export function sanitizeWizardCityInput(raw, max = WIZARD_TEXT_LIMITS.city, opts = {}) {
...
function validateCityField(value, { required = false } = {}) {
  const lengthResult = withRequiredLabel(
    validateLength(value, { required, max: WIZARD_TEXT_LIMITS.city }),
    value,
    "City",
  );
...
export function getPropertyWizardFieldError(field, form = {}) {
  ...
  const propertySubtype = String(form.propertySubtype || "").toLowerCase();

  switch (field) {
    case "title": {
      const title = withRequiredLabel(
        validateLength(form.title, {
          required: true,
          min: 10,
          max: WIZARD_TEXT_LIMITS.title,
        }),
    ...
    case "propertySubtype": {
      if (!propertySubtype) return requiredLabelError("Property subtype");
      const combo = validatePropertyTypeSubtype(propertyType, propertySubtype, {
        required: true,
      });
      return combo.ok ? null : combo.error;
    }
    case "description": {
    ...
    case "area": {
      const area = withRequiredLabel(validateLength(form.area, {
        required: Object.hasOwn(form, "propertyKind"),
        max: WIZARD_TEXT_LIMITS.area,
      }), form.area, "Area / Neighbourhood");
      return area.ok ? null : area.error;
    }
    case "phase": {
      const phase = validateLength(form.phase, {
        required: false,
        max: WIZARD_TEXT_LIMITS.phase,
      });
...
export function validatePropertySubmissionFields(property = {}) {
  ...
  const propertyData = readPropertyData(property.property_data);
  const structured = ["landInfo", "propertyDetails", "commercialInfo", "plotInfo"].some(key => Object.hasOwn(propertyData || {}, key));

  if (!title) {
    errors.push("Property title is required.");
  } else if (title.length < 10) {
    errors.push("Property title must be at least 10 characters.");
  } else if (title.length > WIZARD_TEXT_LIMITS.title) {
    errors.push(`Maximum ${WIZARD_TEXT_LIMITS.title} characters allowed for title.`);
  }

  // Description is optional; validate format only when provided.
  if (description) {
    if (description.length > WIZARD_TEXT_LIMITS.description) {
      errors.push(
        `Maximum ${WIZARD_TEXT_LIMITS.description} characters allowed for description.`,
      );
  ...
  if (structured && !area) errors.push("Area / Neighbourhood is required.");
  if (area && area.length > WIZARD_TEXT_LIMITS.area) {
    errors.push(
      `Maximum ${WIZARD_TEXT_LIMITS.area} characters allowed for area.`,
    );
  }
  if (
    property?.phase &&
    normalizeWhitespace(property.phase).length > WIZARD_TEXT_LIMITS.phase
  ) {
    errors.push(
      `Maximum ${WIZARD_TEXT_LIMITS.phase} characters allowed for phase.`,
    );
  }
  ...
  } else if (address.length > WIZARD_TEXT_LIMITS.address) {
    errors.push(
      `Maximum ${WIZARD_TEXT_LIMITS.address} characters allowed for address.`,
    );
  }

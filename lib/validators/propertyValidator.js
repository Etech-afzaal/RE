...
export function validatePropertyDraftInput(input = {}, opts = {}) {
  ...
  const fields = {
    ...
    city: validateLength(input.city, { required: false, max: 100 }),
    area: validateLength(input.area, { required: false, max: 100 }),
    phase: validateLength(input.phase, { required: false, max: 100 }),
    address: validateLength(input.address, { required: false, max: 300 }),
 ...
 * @returns {{ value: string, limitError: string|null }}
 */
export function sanitizeWizardCityInput(raw, max = 100, opts = {}) {
...
function validateCityField(value, { required = false } = {}) {
  const lengthResult = withRequiredLabel(
    validateLength(value, { required, max: 100 }),
    value,
    "City",
  );
  ...
  switch (field) {
    case "title": {
      const title = withRequiredLabel(
        validateLength(form.title, { required: true, min: 10, max: 150 }),
        form.title,
        "Title",
      );
    ...
    case "area": {
      const area = validateLength(form.area, { required: false, max: 100 });
      return area.ok ? null : area.error;
    }
    case "phase": {
      const phase = validateLength(form.phase, { required: false, max: 100 });
      return phase.ok ? null : phase.error;
    }
    case "address": {
      const address = withRequiredLabel(
        validateLength(form.address, { required: true, min: 10, max: 300 }),
        form.address,
        "Address",
      );
...
export function validatePropertySubmissionFields(property = {}) {
  ...
  if (!title) {
    ...
    errors.push("Maximum 150 characters allowed for title.");
  }
  ...
  if (area && area.length > 100) {
    errors.push("Maximum 100 characters allowed for area.");
  }
  if (property?.phase && normalizeWhitespace(property.phase).length > 100) {
    errors.push("Maximum 100 characters allowed for phase.");
  }
  ...
  if (!address) {
    ...
    errors.push("Maximum 300 characters allowed for address.");
  }

...
function buildAgentInquiryHtml({ agent, customer, property, attribution }) {
  ...
  const attributionBlock = attribution
    ? `
      <p style="margin: 18px 0 6px;"><strong>Attribution:</strong></p>
      <p style="margin: 4px 0;"><strong>Subagent:</strong><br/>${escapeHtml(attribution.subagentName)}</p>
      <p style="margin: 4px 0;"><strong>Marketing Link:</strong><br/>${escapeHtml(attribution.marketingCode)}</p>
    `
    : "";
 ...
 *
 * @param {{ agent_id?: number, property_id?: number, name: string, email: string, phone?: string, subject?: string, message: string, page_url?: string, marketing_ref?: string }} input
 */
export async function createCustomerInquiry(input) {
  ...
  const marketingRef = String(input?.marketing_ref || "").trim();
  if (marketingRef && property?.id) {
    marketingLink = await getMarketingLinkByCode(marketingRef);
    if (
      !marketingLink ||
      Number(marketingLink.property_id) !== Number(property.id) ||
      marketingLink.property_status !== PROPERTY_PUBLIC_STATUS ||
      isAgentOwnMarketingLink(marketingLink) ||
      !marketingLink.subagent_is_active
    ) {
      marketingLink = null;
    }
  }

  let insightLink = null;
  const insightRef = String(input?.insight_ref || "").trim() || marketingRef;
  if (insightRef && property?.id) {
    insightLink = await getMarketingLinkByCode(insightRef);
    if (
      !insightLink ||
      Number(insightLink.property_id) !== Number(property.id) ||
      insightLink.property_status !== PROPERTY_PUBLIC_STATUS ||
      (!isAgentOwnMarketingLink(insightLink) && !insightLink.subagent_is_active)
    ) {
      insightLink = null;
    }
  }
  ...
  const attribution = marketingLink
    ? {
        subagentName: marketingLink.subagent_name,
        marketingCode: marketingLink.unique_code,
      }
    : null;
  const html = buildAgentInquiryHtml({ agent, customer, property, attribution });
  ...
  if (marketingLink) {
    toEmail = String(marketingLink.subagent_email || "").trim();
    if (!toEmail) {
      toEmail = agentEmail;
    } else {
      cc =
        agentEmail && agentEmail.toLowerCase() !== toEmail.toLowerCase()
          ? agentEmail
          : null;
      bcc =
        adminEmail && adminEmail.toLowerCase() !== toEmail.toLowerCase()
          ? adminEmail
          : null;
      if (
        cc &&
        bcc &&
        cc.toLowerCase() === bcc.toLowerCase()
      ) {
        bcc = null;
      }
    }
  ...
  try {
    await sendMail(toEmail, emailSubject, html, mailOptions);
  } catch (err) {
  ...
  if (insightLink) {
    await recordLinkInsight(insightLink.id, "email_sent").catch(() => {});
  }

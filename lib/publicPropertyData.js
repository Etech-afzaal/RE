...
/** Covered floor area label; callers fall back to land size when unavailable. */
export function apartmentCoveredAreaLabel(property) {
  const data = readPropertyData(property.property_data);
  const value = readPropertyData(data?.propertyDetails)?.coveredArea;
  if (!["string", "number"].includes(typeof value) || !Number.isFinite(Number(value)) || Number(value) <= 0) return null;
  return `${String(value).trim()} sqft`;
}
...
/** Card tag only: retain the existing taxonomy for grouping and filtering. */
export function publicPropertyCardTypeLabel(property, fallback) {
  const kind = propertyKind(property.property_type, property.property_subtype);
  const data = readPropertyData(property.property_data);
  if (kind === "commercial") {
    const info = readPropertyData(data?.commercialInfo) || {};
    const type = Object.hasOwn(info, "commercialType") ? info.commercialType : info.type;
    return displayValue(type === "Other" ? info.commercialTypeOther : type) || fallback;
  }
  if (kind === "plots") {
    return displayValue(readPropertyData(data?.plotInfo)?.plotType) || fallback;
  }
  return fallback;
}
...
export function publicPropertyDetails(property) {
  ...
  if (data) {
    ...
    for (const section of names) {
      if (!Object.hasOwn(data, section) && !(section === "propertyDetails" && kind === "commercial" && data.commercialInfo)) continue;
      amenities ??= [];
      ...
      for (const { key, label } of fieldsForSection(section, kind)) {
        ...
        let value = displayValue(source[key]);
        const unit = { front: "ft", depth: "ft", roadWidth: "ft", coveredArea: "sqft" }[key];
        if (unit && /^\d+(?:\.\d+)?$/.test(value)) value = `${value} ${unit}`;
        if (value) rows.push({ label: label.replace(/\s*\((?:ft|sqft)\)$/, ""), value });
      }
      if (section === "propertyDetails") {
        ...
        if (kind === "commercial") {
          ...
          const value = commercial.commercialType === "Other" && displayValue(commercial.commercialTypeOther)
            ? displayValue(commercial.commercialTypeOther)
            : Object.hasOwn(commercial, "commercialType")

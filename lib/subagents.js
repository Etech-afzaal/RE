...
export async function getSubagentsPageByAgent(
  agentId,
  ...
  const searchClause = searchTerm
    ? " AND (LOCATE(?, LOWER(name)) > 0 OR LOCATE(?, LOWER(email)) > 0)"
    : "";
  const queryParams = searchTerm ? [agentId, searchTerm, searchTerm] : [agentId];
  const countRows = await query(
    `SELECT COUNT(*) AS total FROM subagents WHERE agent_id = ? AND is_active = 1${searchClause}`,
    queryParams,
  );
  const totalSubagents = Number(countRows[0]?.total) || 0;
  const totalPages = Math.max(1, Math.ceil(totalSubagents / safePageSize));
  const currentPage = Math.min(safePage, totalPages);
  const offset = (currentPage - 1) * safePageSize;

  const subagents = await query(
    `SELECT ${SUBAGENT_SELECT}
     FROM subagents
     WHERE agent_id = ? AND is_active = 1${searchClause}
     ORDER BY created_at ASC

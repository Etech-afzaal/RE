...
const roads = ["20 ft", "25 ft", "30 ft", "40 ft", "50 ft", "60 ft", "80 ft+"];
const field = (key, label, options) => ({ key, label, options });
export const PROPERTY_DATA_FIELDS = {
  landInfo: [field("front", "Front (ft)"), field("depth", "Depth (ft)"), field("plotPosition", "Plot Position", positions), field("roadWidth", "Road Width", roads), field("dimension", "Dimension")],
  ...
  plotInfo: [
    ...
    field("roadWidth", "Road Width", roads),
...
export function normalizePropertyData(raw, type, subtype, marketingInput = {}) {
  ...
  for (const section of sectionsForKind(kind)) {
    ...
    if (section === "propertyDetails") definitions.push(field("bedrooms", "Bedrooms"), field("bathrooms", "Bathrooms"), field("parking", "Parking", ["Yes", "No"]));
    for (const { key, label, options } of definitions) {
      ...
      if (key === "amenities" && options) {
        if (!Array.isArray(value) || value.length > options.length || value.some(v => !options.includes(v))) return fail(`Choose valid ${label.toLowerCase()}.`, name);
        if (value.length) data[section][key] = [...new Set(value)];

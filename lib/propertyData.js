...
export const PROPERTY_KIND_OPTIONS = {
  sale: ["plots", "file", "house", "commercial", "apartment"],
  rent: ["house", "commercial", "apartment"],
};
export const PROPERTY_KIND_LABELS = {
  house: "House", apartment: "Apartment", commercial: "Commercial", plots: "Plot", file: "File",
};
...
export const PROPERTY_DATA_FIELDS = {
  ...
  commercialInfo: [field("commercialType", "Commercial Type", ["Shop", "Plaza", "Office", "Warehouse", "Building", "Other"]), field("commercialTypeOther", "Other Commercial Type")],
  plotInfo: [
...
export function propertyWizardSteps(kind) {
  ...
  const labels = ["Basic Information", ...(building ? ["Land Info", "Property Details"] : ["Plot Information"]), "Images", "Videos", "Insights & Submission"];
  return { labels, BASIC: 0, LOCATION: 0, LAND: 1, DETAILS: building ? 2 : 1, IMAGES: labels.length - 3, VIDEO: labels.length - 2, ACTIONS: labels.length - 1 };
}
...
export function fieldsForSection(section, kind) {
  ...
  return fields.filter(({ key }) => ["plotType", "amenities"].includes(key));
}
...
export function normalizePropertyData(raw, type, subtype, marketingInput = {}) {
  ...
  for (const section of sectionsForKind(kind)) {
    ...
    for (const { key, label, options } of definitions) {
      if (key === "commercialTypeOther" && source?.commercialType !== "Other") continue;
      ...
      if (key === "amenities" && Array.isArray(value)) {
        ...
        if (parseFloat(value) === 0) return fail("Road Width must be greater than 0.", name);
        ...
        if (Number(value) === 0) return fail(`${label} must be greater than 0.`, name);
  ...
  // Explicitly empty input lists hide sections; migrated NULL means never set.
  for (const key of INSIGHT_FIELDS) {
    const original = Object.hasOwn(marketingInput, key) ? marketingInput[key] : input?.insights?.[key];
    if (Array.isArray(original) && original.length === 0) marketing.data[key] = [];
  }
  data.insights = marketing.data;
  return { ok: true, data: raw == null ? null : data, marketing: marketing.data };
}

/** Merge partial edits into existing JSON; omitted fields are never cleared. */
export function preparePropertyDataSave(body, type, subtype, current = null) {
  let raw = Object.hasOwn(body, "property_data") ? body.property_data : current;
  const incoming = readPropertyData(raw);
  const previous = readPropertyData(current);
  if (incoming && previous) {
    raw = { ...previous, ...incoming };
    for (const [section, values] of Object.entries(incoming)) {
      if (readPropertyData(values) && typeof values !== "string") {
        raw[section] = { ...readPropertyData(previous[section]), ...values };
      }
    }
  }

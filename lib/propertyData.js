...
export function preparePropertyDataSave(body, type, subtype, current = null, currentMarketing = {}) {
  ...
  const marketing = Object.fromEntries(INSIGHT_FIELDS.flatMap(key => {
    if (Object.hasOwn(body, key)) return [[key, body[key]]];
    if (Object.hasOwn(readPropertyData(raw)?.insights || {}, key)) return [];
    return Object.hasOwn(currentMarketing, key) ? [[key, currentMarketing[key]]] : [];
  }));

...
export async function getPublishedPropertiesPage(
  ...
  if (propertyType && propertyType !== "all") {
    filters.push("JSON_VALUE(p.property_data, '$.listing_type' RETURNING CHAR(50)) = ?");
    params.push(propertyType);
  }

  if (city) {
    filters.push(`${propertyLocationSQL("city")} LIKE ?`);
    params.push(`%${city}%`);
  }
  ...
  if (normalizedSearch) {
    ...
    if (tokens.length > 0) {
      const tokenFields =
        `(p.title LIKE ? OR p.location LIKE ? OR ${propertyLocationSQL("city")} LIKE ? OR ${propertyLocationSQL("area")} LIKE ? OR ${propertyLocationSQL("phase")} LIKE ? OR p.description LIKE ? OR a.full_name LIKE ? OR a.company_name LIKE ? OR a.estate_name LIKE ? OR JSON_VALUE(p.property_data, '$.property_type' RETURNING CHAR(50)) LIKE ? OR JSON_VALUE(p.property_data, '$.listing_type' RETURNING CHAR(50)) LIKE ? OR CAST(p.size_value AS CHAR) LIKE ? OR p.size_unit LIKE ?)`;
      filters.push(`(${tokens.map(() => tokenFields).join(" AND ")})`);
  ...
  const properties = await query(
    `SELECT p.*, a.estate_name, a.username, a.full_name AS agent_name,
            a.company_name, a.profile_image, a.office_address
     FROM properties p
     JOIN users a ON a.id = p.agent_id
     WHERE ${where}
     ORDER BY p.created_at DESC

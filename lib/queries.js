...
export async function getAgentDiscoveryCities() {
  return query(
    `SELECT p.city AS name, COUNT(*) AS listingCount
     FROM properties p
     JOIN users a ON a.id = p.agent_id
     WHERE p.status = ?
       AND a.status = ?
       ...
       AND TRIM(p.city) != ''
     GROUP BY p.city
     ORDER BY listingCount DESC, p.city ASC`,
    [PROPERTY_PUBLIC_STATUS, AGENT_LIVE_STATUS],
  );
}
...
export async function getManagedPropertiesPageByAgent(
  ...
  if (normalizedSearch) {
    ...
    filters.push(
      "(p.title LIKE ? OR p.location LIKE ? OR p.city LIKE ? OR p.area LIKE ? OR p.phase LIKE ? OR p.address LIKE ? OR p.description LIKE ?)",
    );

...
export async function getHeroSlides(limit = 5) {
  ...
  // Prefer sale houses for the hero (better wide photos). Rent/plot titles are
  // excluded so tiny plot/apartment crops are less likely to fill the banner.
  const properties = await query(
  ...
  return properties.map((p) => {
    ...
    return {
      ...
      // Same featured image as listing cards so hero slides match For Sale / etc.

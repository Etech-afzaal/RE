...
export async function getPublishedPropertiesPage(
  ...
  if (normalizedSearch) {
    const like = `%${normalizedSearch}%`;
    filters.push(
      `(p.title LIKE ? OR p.location LIKE ? OR ${propertyLocationSQL("city")} LIKE ? OR ${propertyLocationSQL("area")} LIKE ? OR ${propertyLocationSQL("phase")} LIKE ? OR p.description LIKE ? OR a.full_name LIKE ? OR a.company_name LIKE ? OR a.estate_name LIKE ?)`,
    );

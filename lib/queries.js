...
export async function getManagedPropertiesPageByAgent(
  agentId,
  { page = 1, pageSize = 10, status, featured = false, search } = {},
) {
  ...
  const featuredCountRows = await query(
    `SELECT COUNT(*) AS total
     FROM properties
     WHERE agent_id = ? AND is_featured = TRUE AND status = ?`,
    [agentId, PROPERTY_PUBLIC_STATUS],
  );
...
export async function getAgentBrandStats(agentId) {
  const rows = await query(
    `SELECT
       ...
       SUM(CASE WHEN status = 'sold' THEN 1 ELSE 0 END) AS sold,
       SUM(CASE WHEN status IN ('approved', 'under_contract', 'sold') THEN 1 ELSE 0 END) AS totalProjects,
       COUNT(DISTINCT CASE
         WHEN status IN ('approved', 'under_contract', 'sold')

 ...
 * @param {{ allowPasswordReset?: boolean }} [options]
 *   Set allowPasswordReset when calling from the forced password-reset API.
 */
export async function requireAgent(options = {}) {
  const session = await getCurrentUser();
  ...
  if (session.user.mustResetPassword && !options.allowPasswordReset) {
    return {
      session: null,
      user: null,
      error: unauthorized("Password reset required.", 403),
    };
  }

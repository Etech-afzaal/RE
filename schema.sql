...
CREATE TABLE IF NOT EXISTS users (
  ...
  phone VARCHAR(50),
  secondary_phone VARCHAR(50) NULL,                -- optional second number, shown on public pages
  whatsapp_number VARCHAR(50) NULL,                -- separate WhatsApp number; wa.me falls back to phone
...
-- property (subagent_id NULL = the agent's own link), and every page view /
-- phone / WhatsApp / email action on that link is logged as an insight.
-- ---------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS property_marketing_links (
  ...
  property_id INT NOT NULL,
  agent_id INT NOT NULL,
  subagent_id INT NULL,                            -- NULL = the agent's own link
  unique_code VARCHAR(16) NOT NULL,
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  -- NULL-safe key so each property has at most one own-link and one link per subagent.
  subagent_id_key INT GENERATED ALWAYS AS (IFNULL(subagent_id, -1)) STORED,
  PRIMARY KEY (id),
  UNIQUE KEY uq_marketing_links_code (unique_code),
  UNIQUE KEY uq_marketing_links_property_subagent (property_id, subagent_id_key),

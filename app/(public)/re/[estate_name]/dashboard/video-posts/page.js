                      ...
                      <tr key={videoPost.id}>
                        ...
                        <td data-label="Actions">
                          <div className={quickActionStyles.actionGroup}>
                            <PropertyQuickAction
                              icon={SquarePen}
                              tooltip="Edit Video Post"
                              onClick={() => router.push(editHref)}
                            />
                            <ActionMenu
                              ariaLabel={`More actions for ${videoPost.title}`}
                              triggerTooltip="More Actions"
                              onDelete={() => setDeleteTarget(videoPost)}
                              additionalActions={[{
                                label: isPublished ? "Unpublish" : "Publish",
                                icon: isPublished ? Undo2 : Send,
                                onSelect: () => updateStatus(videoPost, isPublished ? "draft" : "published"),
                              }]}
                            />
                          </div>
                        </td>

...
import styles from "./AgentWhatsAppFab.module.css";
...
/**
 * Fixed bottom-left WhatsApp FAB for agent public pages.
 * Renders nothing when the agent has no usable phone number.
 */
export default function AgentWhatsAppFab({ phone, message, label = "WhatsApp" }) {
  ...
  return (
    <a
      ...
      aria-label={`Chat on WhatsApp${label ? ` — ${label}` : ""}`}
    >

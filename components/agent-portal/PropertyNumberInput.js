...
export default function PropertyNumberInput({ value, onChange, max = 999999.99, min = 0, step = 0.01, className, disabled = false, ...props }) {
  ...
  return <div className={styles.numberControl}>
    <input {...props} type="number" className={`${className || ""} ${styles.numberInput}`} value={value ?? ""} min={min} max={max} step={step} disabled={disabled} inputMode={step === 1 ? "numeric" : "decimal"} onKeyDown={e => { if (["e", "E", "+", "-"].includes(e.key)) e.preventDefault(); }} onPaste={paste} onChange={e => {
